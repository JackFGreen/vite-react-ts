...
async function parseJSON<T>(response: Response, config: AjaxCofig): Promise<T> {
	...
	if (config?.httpCodeHandler instanceof Object && typeof config?.httpCodeHandler[status] === 'function') {
...
class Ajax {
	...
	async send<Req, Res>(method: AjaxMethod, url: string, params?: Req, conf?: AjaxCofig): Promise<Res> {
